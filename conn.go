...
func (conn *conn) sendMessage(message *Message) {
	...
	if !found {
		glog.Warningf("Can not found chunk strem id %d", message.ChunkStreamID)
		// Error
		return
	}
...
func (conn *conn) readLoop() {
	...
	defer func() {
		...
		if r := recover(); r != nil {
			if conn.err == nil {
				conn.err = r.(error)
				glog.Warningf("readLoop panic:", conn.err)
			}
		}
		conn.Close()
	...
	for !conn.closed {
		...
		if !found || chunkstream == nil {
			glog.V(1).Infof("New stream 1 csi: %d, fmt: %d\n", csi, vfmt)
		...
		if !found {
			glog.V(1).Infof("New stream 2 csi: %d, fmt: %d, header: %+v\n", csi, vfmt, header)
		}
		conn.inBytes += uint32(n)
		...
		case HEADER_FMT_SAME_STREAM:
			...
			if chunkstream.lastHeader == nil {
				glog.Warningf("A new message with fmt: %d, csi: %d\n", vfmt, csi)
				header.Dump("err")
			} else {
		...
		if remain <= conn.inChunkSize {
			...
			for {
				...
				if err == nil {
					...
					if remain <= uint32(n64) {
						...
						remain -= uint32(n64)
						glog.V(1).Infof("Message continue copy remain: %d\n", remain)
						continue
					}
				}
				...
				glog.V(1).Infof("Message copy blocked!\n")
			}
			// Finished message
			...
			// Unfinish
			glog.V(2).Infof("Unfinish message(remain: %d, chunksize: %d)\n", remain, conn.inChunkSize)
			...
			for {
				...
				if err == nil {
					...
					if remain <= uint32(n64) {
						...
						remain -= uint32(n64)
						glog.V(1).Infof("Unfinish message continue copy remain: %d\n", remain)
						continue
					}
					break
				}
				...
				glog.V(1).Infof("Unfinish message copy blocked!\n")
			}
...
func (conn *conn) CreateMediaChunkStream() (*OutboundChunkStream, error) {
	...
	for index, occupited := range conn.mediaChunkStreamIDAllocator {
		if !occupited {
			...
			glog.V(2).Infof("index: %d, newChunkStreamID: %d\n", index, newChunkStreamID)
...
func (conn *conn) received(message *Message) {
	...
	if message.Type == AGGREGATE_MESSAGE_TYPE {
		...
		for message.Buf.Len() > 0 {
			...
			if err != nil {
				glog.Warningf("conn::received() AGGREGATE_MESSAGE_TYPE read sub type err:", err)
				return
			}
			...
			if err != nil {
				glog.Warningf("conn::received() AGGREGATE_MESSAGE_TYPE read data size err:", err)
				return
			}
			...
			_, err = io.ReadAtLeast(message.Buf, tmpBuf[1:], 3)
			if err != nil {
				glog.Warningf("conn::received() AGGREGATE_MESSAGE_TYPE read timestamp err:", err)
				return
			}
			...
			if err != nil {
				glog.Warningf("conn::received() AGGREGATE_MESSAGE_TYPE read timestamp extend err:", err)
				return
			}
			...
			_, err = io.ReadAtLeast(message.Buf, tmpBuf[1:], 3)
			if err != nil {
				glog.Warningf("conn::received() AGGREGATE_MESSAGE_TYPE read ignore bytes err:", err)
				return
			}
			...
			if err != nil {
				glog.Warningf("conn::received() AGGREGATE_MESSAGE_TYPE copy data err:", err)
				return
			}
			...
			if message.Buf.Len() >= 4 {
				...
				if err != nil {
					glog.Warningf("conn::received() AGGREGATE_MESSAGE_TYPE read previous tag size err:", err)
					return
				}
				tmpBuf[0] = 0
			} else {
				glog.Warningf("conn::received() AGGREGATE_MESSAGE_TYPE miss previous tag size")
				break
			}
		}
	} else {
		...
		case CS_ID_PROTOCOL_CONTROL:
			...
			default:
				glog.V(1).Infof("Unkown message type %d in Protocol control chunk stream!\n", message.Type)
			}
		case CS_ID_COMMAND:
			if message.StreamID == 0 {
				...
				case COMMAND_AMF3:
					...
					if err != nil {
						glog.Warningf("Read first in flex commad err:", err)
						return
					}
					fallthrough
				case COMMAND_AMF0:
					...
					if err != nil {
						glog.Warningf("AMF0 Read name err:", err)
						return
					}
					transactionID, err = amf.ReadDouble(message.Buf)
					if err != nil {
						glog.Warningf("AMF0 Read transactionID err:", err)
						return
					}
					...
					for message.Buf.Len() > 0 {
						...
						if err != nil {
							glog.Warningf("AMF0 Read object err:", err)
							return
						}
						cmd.Objects = append(cmd.Objects, object)
					}
				default:
					glog.V(1).Infof("Unkown message type %d in Command chunk stream!\n", message.Type)
				}
...
func (conn *conn) invokeSetChunkSize(message *Message) {
	if err := binary.Read(message.Buf, binary.BigEndian, &conn.inChunkSize); err != nil {
		glog.Warningf("conn::invokeSetChunkSize err:", err)
	}
	glog.V(1).Infof("conn::invokeSetChunkSize() conn.inChunkSize = %d\n", conn.inChunkSize)
}

func (conn *conn) invokeAbortMessage(message *Message) {
	glog.V(1).Infof("conn::invokeAbortMessage()")
}

func (conn *conn) invokeAcknowledgement(message *Message) {
	glog.V(1).Infof("conn::invokeAcknowledgement(): % 2x\n", message.Buf.Bytes())
}
...
func (conn *conn) invokeUserControlMessage(message *Message) {
	...
	if err != nil {
		glog.Warningf("conn::invokeUserControlMessage() read event type err: %s\n", err.Error())
		return
	}
	switch eventType {
	case EVENT_STREAM_BEGIN:
		glog.V(1).Infof("conn::invokeUserControlMessage() EVENT_STREAM_BEGIN")
	case EVENT_STREAM_EOF:
		glog.V(1).Infof("conn::invokeUserControlMessage() EVENT_STREAM_EOF")
	case EVENT_STREAM_DRY:
		glog.V(1).Infof("conn::invokeUserControlMessage() EVENT_STREAM_DRY")
	case EVENT_SET_BUFFER_LENGTH:
		glog.V(1).Infof("conn::invokeUserControlMessage() EVENT_SET_BUFFER_LENGTH")
	case EVENT_STREAM_IS_RECORDED:
		glog.V(1).Infof("conn::invokeUserControlMessage() EVENT_STREAM_IS_RECORDED")
	case EVENT_PING_REQUEST:
		...
		if err != nil {
			glog.Warningf("conn::invokeUserControlMessage() read serverTimestamp err: %s\n", err.Error())
			return
		}
		...
		if err = binary.Write(respmessage.Buf, binary.BigEndian, &respEventType); err != nil {
			glog.Warningf("conn::invokeUserControlMessage() write event type err:", err)
			return
		}
		if err = binary.Write(respmessage.Buf, binary.BigEndian, &serverTimestamp); err != nil {
			glog.Warningf("conn::invokeUserControlMessage() write streamId err:", err)
			return
		}
		glog.V(1).Infof("conn::invokeUserControlMessage() Ping response")
		conn.Send(respmessage)
	case EVENT_PING_RESPONSE:
		glog.V(1).Infof("conn::invokeUserControlMessage() EVENT_PING_RESPONSE")
	case EVENT_REQUEST_VERIFY:
		glog.V(1).Infof("conn::invokeUserControlMessage() EVENT_REQUEST_VERIFY")
	case EVENT_RESPOND_VERIFY:
		glog.V(1).Infof("conn::invokeUserControlMessage() EVENT_RESPOND_VERIFY")
	case EVENT_BUFFER_EMPTY:
		glog.V(1).Infof("conn::invokeUserControlMessage() EVENT_BUFFER_EMPTY")
	case EVENT_BUFFER_READY:
		glog.V(1).Infof("conn::invokeUserControlMessage() EVENT_BUFFER_READY")
	default:
		glog.V(1).Infof("conn::invokeUserControlMessage() Unknown user control message :0x%x\n", eventType)
	}
}
...
func (conn *conn) invokeWindowAcknowledgementSize(message *Message) {
	...
	if err = binary.Read(message.Buf, binary.BigEndian, &size); err != nil {
		glog.Warningf("conn::invokeWindowAcknowledgementSize read window size err:", err)
		return
	}
	conn.inWindowSize = size
	glog.V(1).Infof("conn::invokeWindowAcknowledgementSize() conn.inWindowSize = %d\n", conn.inWindowSize)
}
...
func (conn *conn) invokeSetPeerBandwidth(message *Message) {
	...
	if err = binary.Read(message.Buf, binary.BigEndian, &conn.inBandwidth); err != nil {
		glog.Warningf("conn::invokeSetPeerBandwidth read window size err:", err)
		return
	}
	conn.inBandwidth = size
	var limit byte
	if limit, err = message.Buf.ReadByte(); err != nil {
		glog.Warningf("conn::invokeSetPeerBandwidth read limit err:", err)
		return
	}
	conn.inBandwidthLimit = uint8(limit)
	glog.V(1).Infof("conn.inBandwidthLimit = %d/n", conn.inBandwidthLimit)
}

func (conn *conn) invokeCommand(cmd *Command) {
	glog.V(1).Infof("conn::invokeCommand()")
	conn.handler.OnReceivedRtmpCommand(conn, cmd)
}

func (conn *conn) SetStreamBufferSize(streamId uint32, size uint32) {
	glog.V(1).Infof("conn::SetStreamBufferSize(streamId: %d, size: %d)\n", streamId, size)
	...
	if err := binary.Write(message.Buf, binary.BigEndian, &eventType); err != nil {
		glog.Warningf("conn::SetStreamBufferSize write event type err:", err)
		return
	}
	if err := binary.Write(message.Buf, binary.BigEndian, &streamId); err != nil {
		glog.Warningf("conn::SetStreamBufferSize write streamId err:", err)
		return
	}
	if err := binary.Write(message.Buf, binary.BigEndian, &size); err != nil {
		glog.Warningf("conn::SetStreamBufferSize write size err:", err)
		return
	}
	conn.Send(message)
}
...
func (conn *conn) SetChunkSize(size uint32) {
	glog.V(1).Infof("conn::SetChunkSize(size: %d)\n", size)
	message := NewMessage(CS_ID_PROTOCOL_CONTROL, SET_CHUNK_SIZE, 0, 0, nil)
	if err := binary.Write(message.Buf, binary.BigEndian, &size); err != nil {
		glog.Warningf("conn::SetChunkSize write event type err:", err)
		return
	}
...
func (conn *conn) SetWindowAcknowledgementSize() {
	glog.V(1).Infof("conn::SetWindowAcknowledgementSize")
	...
	if err := binary.Write(message.Buf, binary.BigEndian, &conn.outWindowSize); err != nil {
		glog.Warningf("conn::SetWindowAcknowledgementSize write window size err:", err)
		return
	}
...
func (conn *conn) SetPeerBandwidth(peerBandwidth uint32, limitType byte) {
	glog.V(1).Infof("conn::SetPeerBandwidth")
	// Request window acknowledgement size
	...
	if err := binary.Write(message.Buf, binary.BigEndian, &peerBandwidth); err != nil {
		glog.Warningf("conn::SetPeerBandwidth write peerBandwidth err:", err)
		return
	}
	if err := message.Buf.WriteByte(limitType); err != nil {
		glog.Warningf("conn::SetPeerBandwidth write limitType err:", err)
		return
	}
	message.Size = uint32(message.Buf.Len())
	conn.Send(message)
...
func (conn *conn) SendUserControlMessage(eventId uint16) {
	glog.V(1).Infof("conn::SendUserControlMessage")
	...
	if err := binary.Write(message.Buf, binary.BigEndian, &eventId); err != nil {
		glog.Warningf("conn::SendUserControlMessage write event type err:", err)
		return
	}
	conn.Send(message)
}
